...
http.createServer(function (request, response) {
   ...
   if(pathname == "/Childers_index2.html")
   {
...
function readQueryString(queryObject) {
   ...
   if (queryObject != null) {
      ...
      let mEmail = queryObject.myEmail;
      let pNumber = queryObject.phoneNumber;
      if (fName != undefined && lName != undefined && mEmail !== undefined && pNumber !== undefined) {
         fullName = fName + " " + lName + "</br>" + mEmail + " " + pNumber; //break?
         // change the file
         updateFile(fullName); //maybe change this or add another one for the email and phone
      }
...
{
    // open the html file
   fs.readFile('Childers_index2.html', 'utf8', (err,html)=>{
      if(err){
         throw err;
      }
      ...
      // create the stream
      var writerStream = fs.createWriteStream("Childers_index2.html");
